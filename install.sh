...
install_cursor() {
  local src="$SRC_CURSOR"
  [[ "$USE_GPT" == true ]] && src="$SRC_CURSOR_GPT"
  mkdir -p "$DEST_CURSOR"
  for f in "$src"/*.mdc; do
    [[ -f "$f" ]] || continue
...
install_windsurf() {
  local src="$SRC_WINDSURF"
  [[ "$USE_GPT" == true ]] && src="$SRC_WINDSURF_GPT"
...
print_post_install() {
  local style="rich"
  [[ "$USE_GPT" == true ]] && style="sparse (GPT-optimized)"

  echo ""
  echo "=== POST-INSTALL: REQUIRED IDE CONFIGURATION ==="
  echo ""
  echo "  Style installed: $style"
  echo ""
  ...
  for target in "$@"; do
    case "$target" in
      ...
      cursor)
        ...
        echo "    2. Add 00-init as the FIRST rule file (alwaysApply: true)."
        echo "       This is the global runtime — it must load on every request."
        echo "    3. Add 01-router as the second rule (alwaysApply: true)."
        echo "       The router triages every request to the right expert."
        echo "    4. Expert rules (expert-*.mdc) are auto-attached by Cursor"
        echo "       when their description matches the conversation context."
        echo "    Files are in: ~/.cursor/rules/"
        echo ""
        ;;
      windsurf)
        echo "  Windsurf (IMPORTANT)"
        echo "    1. Open Windsurf > Customizations > Rules"
        echo "    2. Verify 00-init.md has trigger: always in its frontmatter."
        echo "       This is the global runtime — it must load on every request."
        echo "    3. Verify 01-router.md has trigger: always in its frontmatter."
        echo "       The router triages every request to the right expert."
        echo "    4. Expert rules use trigger: model_decision — Cascade reads"
        echo "       them automatically when the description matches."
        echo "    Files are in: ~/.windsurf/rules/"
        echo ""
        ;;
      codex)
        echo "  Codex"
...
echo "    Targets: ${TARGETS[*]}"
[[ "$USE_GPT" == true ]] && echo "    Style:   sparse (GPT-optimized)" || echo "    Style:   rich (Claude-optimized, with GPT adaptation preamble)"
echo ""
