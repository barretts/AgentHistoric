...
**Fail-Closed Enforcement:** Every `run_command` invocation MUST append `> .logs/run-<slug>-$(date +%s).log 2>&1` (or `| tee .logs/run-<slug>-$(date +%s).log`). Commands without one of these suffixes are non-compliant. Inline stdout capture is forbidden except for one-line probes (`echo`, `pwd`, `which`) that never produce failure output. Do not use heredocs (`<<EOF`, `<<'NODE'`, `<<PY`) or multi-line terminal payloads; create a script file with file-edit tools and execute it instead.
