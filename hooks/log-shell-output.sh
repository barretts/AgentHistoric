...
if printf '%s' "$CMD" | grep -qiE "$LOG_PATH" && printf '%s' "$CMD" | grep -qiE "$GENERATED_LOG_VALUE"; then
  USER_MSG="Log filename uses a shell-generated value. The agent should choose a literal .logs/ filename, launch the command in background/non-blocking mode, and poll status before inspecting logs. See Tenet 3."
  AGENT_MSG="TENET 3 REWRITE REQUIRED: choose a literal log filename before re-running, for example .logs/run-test-1.log. For retries, use .logs/run-test-2.log. Do not use \$(date), \$(pwd), mktemp, backticks, or shell-generated values for log filenames. Launch the logged command in background/non-blocking mode, poll command status until completion, then inspect the saved log. POSIX pattern: mkdir -p .logs && your command > .logs/run-<slug>-1.log 2>&1; run it non-blocking; poll status; tail -n 50 .logs/run-<slug>-1.log. PowerShell pattern: New-Item -ItemType Directory -Force .logs | Out-Null; your command *> .logs/run-<slug>-1.log; run it non-blocking; poll status; Get-Content -Tail 50 .logs/run-<slug>-1.log."
...
if printf '%s' "$CMD" | grep -qE "$DENYLIST"; then
  USER_MSG="Long-running shell command without a persistent log redirect. The agent should rewrite and re-issue it with logging, launch it in background/non-blocking mode, and poll status before asking for human intervention. See Tenet 3."
  AGENT_MSG="TENET 3 REWRITE REQUIRED: rewrite and re-issue this command with persistent logging instead of asking the human to accept data-loss risk. Choose a literal filename before re-running, for example .logs/run-test-1.log. For retries, use .logs/run-test-2.log. Launch the logged command in background/non-blocking mode, poll command status until completion, then inspect the saved log. POSIX pattern: mkdir -p .logs && your command > .logs/run-<slug>-1.log 2>&1; run it non-blocking; poll status; tail -n 50 .logs/run-<slug>-1.log. PowerShell pattern: New-Item -ItemType Directory -Force .logs | Out-Null; your command *> .logs/run-<slug>-1.log; run it non-blocking; poll status; Get-Content -Tail 50 .logs/run-<slug>-1.log."
  emit_nudge "$USER_MSG" "$AGENT_MSG" "$MODE"
fi
