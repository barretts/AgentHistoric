...
export function generateArtifacts(system) {
  ...
  artifacts.set(
    path.join(".cursor", "rules", "00-init.mdc"),
    renderCursorInit(system)
  );
  artifacts.set(
    path.join(".cursor", "rules", "01-router.mdc"),
    renderCursorRouter(system)
  );
  artifacts.set(path.join(".codex", "AGENTS.md"), renderAgents(system));

  for (const expert of system.experts) {
    artifacts.set(
      path.join(".cursor", "rules", `${expert.id}.mdc`),
      renderCursorExpert(system, expert)
    );
    artifacts.set(
...
export function renderCursorInit(system) {
  ...
  return (
    ...
    toHeadingList(system.experts.map((expert) => `${expert.id}: ${expert.title}`)) +
    `\n`
  );
}

export function renderCursorRouter(system) {
...
export function renderCursorExpert(system, expert) {
...
export function renderAgents(system) {
  ...
  return (
      ...
      system.router.routingHeuristics.map(
        (heuristic) =>
          `${heuristic.domain} -> ${heuristic.experts
            .map((expertId) => `.codex/skills/${skillPathForExpert(system, expertId)}`)
            .join(" -> ")}`
      )
    ) +
    `\n\n## Global Rules\n\n` +
    ...
    toHeadingList(
      system.experts.map(
        (expert) => `.codex/skills/${expert.codexSkillDir}: ${humanizeExpertId(expert.id)}`
      )
    ) +
    `\n`
  );
...
function toHeadingList(items) {
  return items.map((item) => `- ${item}`).join("\n");
}

function skillPathForExpert(system, expertId) {
  return system.experts.find((expert) => expert.id === expertId)?.codexSkillDir || expertId;
}
